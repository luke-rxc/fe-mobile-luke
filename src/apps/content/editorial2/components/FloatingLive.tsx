...
import type { ContentLiveModel } from '../models';
...
export const FloatingLiveComponent = forwardRef<HTMLDivElement, FloatingLiveProps>(
  ({ className, live, direction, initFollowed, onChangeLiveStatus, onLiveLink }, ref) => {
     ...
     * 슬라이드 애니메이션 시작시
     */
    const handleSlideMoveStart = (swiper: Swiper) => {
...
export const FloatingLive = styled(FloatingLiveComponent)`
  @keyframes floatingDisappearAnimation {
    from {
      opacity: 1;
    }
  ...
  @keyframes floatingAppearAnimation {
    from {
      opacity: 0;
    }
  ...
  *[stroke] {
  ...
  &.is-changed {
    & .live-wrapper {
      animation: floatingDisappearAnimation 0.3s forwards;
    }

    & .btn-wrapper {
      display: block;
      opacity: 0;
      animation: floatingAppearAnimation 0.3s 0.3s forwards;
    }
  }
