...
import type { ContentInfoModel, LandingActionModel } from '../models';
...
export const useLogService = () => {
  ...
  const logContentInit = ({
    ...
    const logParams = {
      ...
      contents_type: getContentType(contentType),
  ...
  // 헤더 컴포넌트 노출
  const logPresetHeaderInit = (contentInfo: ContentInfoModel) => {
  ...
  // 인터렉티브 미디어 컴포넌트 노출
  const logPresetMediaInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex, presetType } = contentInfo;
    const logParams: {
      ...
      interactive_media_index: `${contentIndex + 1}`,
  ...
  // 미디어뷰어 컴포넌트 노출
  const logPresetMediaViewerInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex, presetType } = contentInfo;
    const logParams: {
      ...
      media_viewer_index: `${contentIndex + 1}`,
      component_type: getPresetType(presetType),
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
  ...
  const logPresetMediaViewerSlide = (contentInfo: ContentInfoModel, mediaIndex: number) => {
    const { contentId, contentName, contentType, contentIndex, presetType } = contentInfo;
    const logParams: {
  ...
  // 미디어뷰어 비디오 재생 조회
  const logPresetMediaViewerVideoView = (contentInfo: ContentInfoModel, viewTime: number, videoIndex: number) => {
    const { contentId, contentName, contentType, contentIndex, presetType } = contentInfo;
    const logParams: {
      media_viewer_index: string;
      component_type: string;
  ...
  // 미디어 뷰어 탭
  const logPresetMediaViewerTab = (contentInfo: ContentInfoModel, actions: LandingActionModel, viewerIndex: number) => {
    const { contentId, contentName, contentType, contentIndex, presetType } = contentInfo;
  ...
  // 미디어뷰어 음소거 변경
  const logPresetMediaViewerTabMute = (contentInfo: ContentInfoModel, viewerIndex: number, state: boolean) => {
    const { contentId, contentName, contentType, contentIndex, presetType } = contentInfo;
    const logParams: {
      component_type: string;
      contents_id: string;
  ...
  // 네비게이션 컴포넌트 노출
  const logPresetNavigationInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      ...
      navigation_component_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
    };
  ...
  // 네비게이션 컴포넌트 탭 클릭
  const logPresetNavigationTab = (contentInfo: ContentInfoModel, tabIndex: number, tabTitle: string) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      navigation_component_index: string;
      contents_type: string;
  ...
  // 텍스트 컴포넌트 노출
  const logPresetTextInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      ...
      text_component_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
    };
  ...
  // 텍스트 음소거 변경
  const logPresetTextTabMute = (contentInfo: ContentInfoModel, state: boolean) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      text_component_index: string;
      contents_id: string;
  ...
  // 라이브 편성 컴포넌트 노출
  const logPresetLiveInit = (contentInfo: ContentInfoModel & { componentType: string }) => {
    const { contentId, contentName, contentType, contentIndex, componentType } = contentInfo;
    const logParams: {
      live_component_index: string;
      contents_id: string;
      contents_name: string;
      contents_type: string;
      component_type: string;
    } = {
      live_component_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
      component_type: componentType,
    };
    debugLog(LogEventTypes.LogPresetLiveInit, logParams);
    tracking.logEvent({
      name: LogEventTypes.LogPresetLiveInit,
      parameters: logParams,
      targets: {
        web: [WebLogTypes.MixPanel],
        app: [AppLogTypes.MixPanel, AppLogTypes.MoEngage],
  ...
  // 라이브 편성 컴포넌트 내 알림신청(팔로우) 완료시
  const logPresetLiveFollow = ({
    contentId,
    contentName,
    showroomId,
    showroomName,
    ...
    debugLog(LogEventTypes.LogPresetLiveFollow, logParams);
    tracking.logEvent({
      name: LogEventTypes.LogPresetLiveFollow,
      parameters: logParams,
      targets: {
        web: [WebLogTypes.MixPanel, WebLogTypes.Branch],
  ...
  // 라이브 편성 컴포넌트 내 알림신청 취소(팔로우 취소) 완료시
  const logPresetLiveUnFollow = ({
    contentId,
    contentName,
    showroomId,
    showroomName,
    ...
    debugLog(LogEventTypes.LogPresetLiveUnFollow, logParams);
    tracking.logEvent({
      name: LogEventTypes.LogPresetLiveUnFollow,
      parameters: logParams,
      targets: {
        web: [WebLogTypes.MixPanel],
        app: [AppLogTypes.MixPanel, AppLogTypes.MoEngage],
  ...
  // 상품 전시 컴포넌트 노출
  const logPresetDealInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex, presetType } = contentInfo;
    const logParams: {
      ...
      goods_component_index: `${contentIndex + 1}`,
      component_type: getPresetType(presetType),
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
  ...
  const logPresetDealGoodsTab = ({
    ...
    const logParams: {
      ...
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: contentType.toLowerCase(),
      goods_component_index: `${contentIndex + 1}`,
      goods_id: `${goodsId}`,
  ...
  // 이미지 컴포넌트 노출
  const logPresetImageViewerInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      ...
      image_component_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
    };
  ...
  // 이미지 탭
  const logPresetImageViewerTab = (contentInfo: ContentInfoModel, actions: LandingActionModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const landingType = getLandingType(actions);
    const landingValue = actions.value;
    const logParams: {
  ...
  // 버튼 컴포넌트 노출
  const logPresetCTAInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      ...
      button_component_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
    };
  ...
  const logPresetCTAButtonTab = ({
    ...
    const { contentId, contentName, contentType } = contentInfo;
    const logParams: {
      contents_id: string;
      contents_name: string;
      contents_type: string;
  ...
  const logPresetDrawInit = (
    ...
    const logParams: {
      ...
      raffle_component_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
      event_id: `${eventId}`,
  ...
  // 쿠폰다운 컴포넌트 노출
  const logPresetCouponDownInit = (contentInfo: ContentInfoModel, { couponType }: { couponType: string }) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      ...
      coupon_component_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
  ...
  // 쿠폰다운 성공 시
  const logPresetCouponDownComplete = (contentInfo: ContentInfoModel, { couponType }: { couponType: string }) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      coupon_component_index: string;
      contents_id: string;
  ...
  // 푸터 컴포넌트 노출
  const logPresetFooterInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType } = contentInfo;
    const logParams: {
      contents_id: string;
      contents_name: string;
  ...
  // 플레이뷰어 컴포넌트 노출
  const logPresetPlayViewerInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      ...
      play_viewer_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
    };
  ...
  // 댓글 컴포넌트 진입
  const logPresetReplyInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentType } = contentInfo;
    const logParams: {
      contents_code: string;
      contents_type: string;
    } = {
      contents_code: `${contentId}`,
      contents_type: getLogContentType(contentType),
    };
  ...
  // 투표 컴포넌트 노출
  const logPresetVoteInit = (contentInfo: ContentInfoModel, { voteId }: { voteId: number }) => {
    const { contentId, contentName, contentType, contentIndex, contentCode } = contentInfo;
    const logParams: {
      ...
      vote_component_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_code: contentCode,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
  ...
  // 투표 인증서 보기 버튼 클릭
  const logPresetVoteCertificationButtonTab = (contentInfo: ContentInfoModel, { voteId }: { voteId: number }) => {
    const { contentId, contentName, contentType, contentIndex, contentCode } = contentInfo;
    const logParams: {
      vote_component_index: string;
      contents_id: string;
  ...
  // 배너 컴포넌트 노출
  const logPresetBannerInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      ...
      banner_component_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
    };
  ...
  // 배너 탭
  const logPresetBannerTab = (contentInfo: ContentInfoModel, actions: LandingActionModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const landingValue = actions.value;
    const logParams: {
      banner_component_index: string;
  ...
  // 혜택 상품 컴포넌트 노출
  const logPresetBenefitGoodsInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex, presetType } = contentInfo;
    const logParams: {
      ...
      benefit_goods_index: `${contentIndex + 1}`,
      component_type: getPresetType(presetType),
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
  ...
  // 혜택 리스트 컴포넌트 노출
  const logPresetBenefitListInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      ...
      benefit_list_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
    };
  ...
  // 임베드 비디오 컴포넌트 노출
  const logPresetEmbedVideoInit = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      ...
      embed_component_index: `${contentIndex + 1}`,
      contents_id: `${contentId}`,
      contents_name: contentName,
      contents_type: getLogContentType(contentType),
    };
  ...
  // 임베드 비디오 컴포넌트 탭
  const logPresetEmbedVideoTab = (contentInfo: ContentInfoModel) => {
    const { contentId, contentName, contentType, contentIndex } = contentInfo;
    const logParams: {
      embed_component_index: string;
      contents_id: string;
...
const getContentType = (type: ContentType): string => {
  switch (type) {
    case ContentType.STORY:
      return 'story';
    case ContentType.TEASER:
