...
import type { ContentInfoModel, VoteItemModel } from '../models';
...
export const usePresetVoteAService = ({
  ...
  const { signIn, open, showToastMessage } = useWebInterface();
  const { logPresetVoteCertificationButtonTab } = useLogService();
  const [voteButtonActive, setVoteButtonActive] = useState(true); // 버튼 상태
  const [voteItemList, setVoteItemList] = useState(list); // 투표리스트
  const { contentCode, contentType } = contentInfo;
  ...
  const handleTapCertification = useCallback(async () => {
    logPresetVoteCertificationButtonTab(contentInfo, { voteId });
    if (!getIsLogin()) {
