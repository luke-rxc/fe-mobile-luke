...
import type { ContentInfoModel, VoteItemModel, VoteReceiveModel } from '../models';
...
export const usePresetVoteItemService = ({
  ...
  const { signIn, open, receiveValues, showToastMessage } = useWebInterface();
  ...
  const { mutateAsync: handleVote, isLoading: isVoteLoading } = useMutation(
    (itemId: number) => postVoteItem(voteId, itemId),
    {
      onSuccess: (res) => {
        ...
        logPresetVoteComplete(contentInfo, {
          voteId,
          voteItemId: vote.id,
