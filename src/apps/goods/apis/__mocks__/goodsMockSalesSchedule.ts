...
endDate.setMinutes(newDate.getMinutes() + 1);

const goodsMockSalesSchedule: GoodsSchema = {
  category: [
    {
      one: {
        id: 3,
  ...
  shipping: {
    ...
    exportingDisplayText: '주문제작 배송 3일 소요',
  },
  showRoom: {
    id: 1,
    code: 'nike',
...
export const goodsMockSalesScheduleApi = ({ goodsId, showRoomId = 0 }: GoodsParam): ReturnType<typeof getGoods> =>
  new Promise((resolve) => {
    window.setTimeout(() => {
      resolve(goodsMockSalesSchedule);
    }, 1000);
  });
