...
import { BubbleType, SelectedInfoValuesType, SelectedPriceType } from '../types';
...
interface Props {
  ...
  priceDisplayMonth: number;
  onTransitionEnd?: (event: React.TransitionEvent) => void;
}

export const DatePickerBubbleV2 = forwardRef<HTMLDivElement, Props>(
  ({ priceRefs, index, bubble, price, priceDisplayMonth, onTransitionEnd: handleTransitionEnd }, ref) => {
    ...
    const { type, textPosition, isShow, size, left } = bubble;

    const visible = isShow && index === priceDisplayMonth;
