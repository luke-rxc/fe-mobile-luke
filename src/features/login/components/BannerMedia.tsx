...
import { forwardRef, useRef, useState, useMemo, useEffect, SyntheticEvent } from 'react';
...
export const BannerMedia = styled(
  forwardRef<HTMLDivElement, BannerMediaProps>(
    ({ type, path, extension, blurHash, poster, inView, onVideoError, ...props }, ref) => {
      ...
      useEffect(() => {
        if (video.current) {
           ...
           * => play()에 catch 적용
           */
          inView && isReadied ? video.current.play().catch(() => {}) : video.current.pause();
        }
          ...
          {(type === 'VIDEO' || isStreaming) && (
            <div className={classnames('banner-video', { 'is-readied': isReadied })}>
              ...
              <video
                ...
                loop
                playsInline
                onLoadedMetadata={handleCanPlayVideo}
                {...(!isStreaming && {
                  src: path,
                  type: 'video/mp4',
