...
import { BrandListItemLarge as Component, BrandListItemLargeProps as Props } from '@pui/brandListItemLarge';
...
export interface BrandListItemLargeProps extends Omit<Props, 'onChangeFollow'> {
  // Follow 클릭 콜백
...
/**
 * useWebInterface의 subscribeShowroom가 연동된 BrandListItemLarge 컴포넌트
 */
export const BrandListItemLarge = styled(
  forwardRef<HTMLDivElement, BrandListItemLargeProps>(
    ({ followed, showroomId, showroomCode, onClickFollow, onChangeFollow, ...other }, ref) => {
      ...
      const handleClickFollow: Props['onChangeFollow'] = (
        e,
        { showroomId: id, showroomCode: code, title: name, followed: followState },
      ) => {
        onClickFollow?.({ id, code, name, follow: !!followState });
      };
